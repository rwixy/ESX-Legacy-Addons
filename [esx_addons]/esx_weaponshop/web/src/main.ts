...
fetchNui<ReadyResponse>('ready', {})
  .then(response => {
    if (response.ok && response.data?.theme) {
      ...
      console.warn('[esx_weaponshop] Failed to load theme from server, using defaults', response.error);
    }
  })
  .catch(error => {
    console.error('[esx_weaponshop] Critical error loading theme:', error);
